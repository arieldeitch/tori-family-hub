...
- **Quality gates, all re-verified locally on 2026-07-31:** typecheck 0 · lint **0 errors / 7 warnings** (6 shadcn `react-refresh` + 1 unused `eslint-disable` in `src/lib/pilot/runtimeConfig.ts:95`, both pre-existing) · **app tests 210/210 across 24 files** · build ✓ · `routes:check` ✓ · `check:client-secrets` ✓ (564 files) · `check:pilot-privacy` ✓ (369 files) · `pilot:test:hosted-guard` ✓ (26/26) · **structural pgTAP 302/302 across 11 files** · **behavioural RLS pgTAP 211/211 across 8 files** · **34/34 Auth-backed integration assertions** · **29/29 pilot bootstrap assertions** · `db:verify` ✓ end to end.
...
- **302 structural pgTAP tests across 11 files** and **211 behavioural RLS pgTAP tests across 8 files** pass, up from 181/9 and 117/6. Every behavioural test asserts `current_user = 'authenticated'` first, because pgTAP connects as a `BYPASSRLS` owner.
- **Two WP3/WP4 assertions were rescoped**, not weakened: the "no business-module tables" guard now proves the WP5C rotation tables have not leaked in early, and the private-helper assertion now also inspects `WITH CHECK` — an INSERT policy has no `USING` clause and had been passing vacuously.
...
- **Open question deferred to WP5D:** task reads use the role-agnostic `is_active_household_member`, so a guest or service provider would read the whole chore list, unlike WP4's narrowed `member_profiles` reads. Left as authored, pinned by an explicit test, recorded in ADR-040. The pilot household has no such profile, so nothing is exposed today.
