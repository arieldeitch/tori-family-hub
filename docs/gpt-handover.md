...
- **The next action is a hosted Supabase session:** re-authenticate the CLI to the account owning `tori-family-pilot` (it has reverted), run `pilot:week:hosted`, publish from Lovable, and verify the live flow with the pilot password.
