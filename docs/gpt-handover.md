...
3. Sanity gates: `bun install --frozen-lockfile` → `typecheck` → `lint` (0 errors, 6 warnings) → `test` (210) → `build` → `routes:check` → `check:client-secrets` → `check:pilot-privacy`; database side: `bun run db:verify` (reset → type freshness → smoke → 310 structural pgTAP → Auth fixtures → 261 behavioural pgTAP → 34 integration assertions → 29 pilot assertions → cleanup).
