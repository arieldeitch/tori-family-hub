...
- ⛔ **BLOCKED — hosted week not yet generated.** The Supabase CLI has reverted to a different account: `supabase projects list` no longer includes `tori-family-pilot` and every project shows `linked: false`. `pilot:week:hosted` is written, guarded and locally proven, but cannot run until the CLI is re-authenticated to the owning account.
- ⛔ **BLOCKED — live verification.** Lovable has not rebuilt since the WP5D merge, and the hosted pilot adult's password is not available in this environment.
