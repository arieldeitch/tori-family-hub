...
- ✅ **Hosted deployment complete (2026-08-01).** WP5B and WP5C applied to `tori-family-pilot` through the normal migration workflow; hosted schema, RLS, grants, helpers, idempotency indexes and append-only triggers all verified; the three approved chores loaded through the guarded, idempotent `pilot:chores:hosted`. Email sign-in is enabled with signup still closed.
- ✅ **Live regression fixed (2026-08-01).** The root route no longer traps signed-in people on the pilot screen; the full application shell and navigation are restored, and the weekly chores are a module at `/chores` (ADR-045).
- ✅ **Week generation exists.** `pilot:week` / `pilot:week:hosted` generate dated occurrences, assignments and rotation decisions from `shifts.v1`, proven idempotent locally (run 3 created 0, totals unchanged).
- ✅ **Hosted week generated (2026-08-01).** `task_instances`, `task_assignments` and `rotation_assignment_log` went **0 → 68**. The current week has **17 occurrences across all seven days**, every one assigned with a reason code and algorithm version. Idempotent on hosted (runs 2 and 3 created 0).
- ⛔ **BLOCKED — Lovable has not rebuilt.** The live origin still serves the pre-WP5D frontend, so the restored navigation and the `/chores` module are not yet live. No Lovable interface is authenticated in this environment, so Publish cannot be triggered from here.
- ⛔ **BLOCKED — live sign-in verification.** The hosted pilot adult's password is not available in this environment.
